 ...
 */
public class HealthExpenditurePerCapita implements ComputeAnalysis{
	/**
	...
	public void computeAnalysis(UserRequest request) {
		...
		Data analysisOne = new FetchData(request, "SH.XPD.CHEX.PC.CD").getData();
		Data analysisTwo = new FetchData(request, "SP.DYN.IMRT.IN").getData();
		...
		model.setAnalysisType("Current Health Expenditure vs Mortality Rate");
		...
		model.setSeries1("Current Health Expenditure per capita (current US$)");
		model.setSeries2("Mortality Rate infant (per 1,000 live births)");
