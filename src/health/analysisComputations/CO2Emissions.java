 ...
 */
public class CO2Emissions implements ComputeAnalysis{
	/**
	...
	public void computeAnalysis(UserRequest request) {
		...
		Data analysisTwo = new FetchData(request, "EG.USE.PCAP.KG.OE").getData();
		Data analysisThree = new FetchData(request, "EN.ATM.PM25.MC.M3").getData();
		
		// Initializing double array to be used to hold values from data
		double[] analysisOneValues;
		double[] analysisTwoValues;
		double[] analysisThreeValues;
		...
		analysisThreeValues = analysisThree.getValues();
		
		// 2D Array that hold values arrays from each respective WorldBank API call
		double[][] analysisValues = {analysisOneValues, analysisTwoValues, analysisThreeValues};
		...
		model.setAnalysisType("CO2 Emissions vs Energy Use vs PM2.5 air pollution");
		...
		model.setNumOfSeries(3);
		// Setting specific series names based off analysis type
		model.setSeries1("CO2 emissions (metric tons per capita)");
		model.setSeries2(" PM2.5 air pollution, mean annual exposure (micrograms per cubic meter)");
		model.setSeries3("Energy use (kg of oil equivalent per capita) ");
